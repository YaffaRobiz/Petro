...
export default function TasksTable({ tasks, currentOdometer, vehicleId, licensePlate, lastServicedByType, logInfoById }: {
  tasks: ServiceTask[]; currentOdometer: number; vehicleId: string; licensePlate: string
  lastServicedByType: LastServicedMap; logInfoById: Record<string, LogInfo>
}) {
  ...
  const statusBtn = (f: Filter, label: string) => (
    <button onClick={() => setFilter(f)} className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${filter === f ? "bg-gray-900 dark:bg-white text-white dark:text-gray-900" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"}`}>{label}</button>
  )
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 px-5 py-2.5 border-b border-gray-50 dark:border-gray-800 overflow-x-auto">
          {categoryOptions.map(c => (
            <button key={c.id} onClick={() => setCategoryFilter(c.id)}
              className={`whitespace-nowrap px-2.5 py-1 rounded-full text-[11px] font-medium transition-colors flex-shrink-0 ${categoryFilter === c.id ? "bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"}`}>
              {c.label}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-gray-400 dark:text-gray-500">
          <p className="text-sm font-medium">{filter === "all" && categoryFilter === "all" ? "No services added yet" : "No matching services"}</p>
        </div>
      ) : (
        filtered.map(task => (
          <TaskRow key={task.id} task={task} currentOdometer={currentOdometer} vehicleId={vehicleId} licensePlate={licensePlate}
